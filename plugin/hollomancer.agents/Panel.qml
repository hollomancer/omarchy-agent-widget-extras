...
Panel {
  ...
  // ------------------------------------------------------ openrouter models
  //
  // OpenRouter has no single "current model" the way a subscription CLI
  // does, so the panel lets you pick one and persists the choice to a state
  // file omarchy-agent-launch-openrouter reads when the bar icon launches it.

  readonly property bool isOpenRouter: !!provider && provider.providerId === "openrouter"
  readonly property string openRouterCacheDir: (Quickshell.env("XDG_CACHE_HOME") || home + "/.cache") + "/omarchy/agent-usage"
  readonly property string openRouterStateDir: (Quickshell.env("XDG_STATE_HOME") || home + "/.local/state") + "/omarchy/agents"

  property var openRouterModels: []
  property string openRouterModelFilter: ""
  property string openRouterSelectedModelId: ""
  property string openRouterSelectedModelName: ""

  readonly property var openRouterFilteredModels: {
    var query = openRouterModelFilter.trim().toLowerCase()
    var list = openRouterModels
    if (query !== "") {
      list = list.filter(function(m) {
        return String(m.id).toLowerCase().indexOf(query) >= 0 || String(m.name).toLowerCase().indexOf(query) >= 0
      })
    }
  ...
  function selectOpenRouterModel(id, name) {
    openRouterSelectedModelId = id
    openRouterSelectedModelName = name
  ...
  function parseOpenRouterSelection(raw) {
    try {
      var parsed = JSON.parse(raw)
      if (parsed && parsed.id) {
        openRouterSelectedModelId = String(parsed.id)
        openRouterSelectedModelName = String(parsed.name || parsed.id)
      }
    } catch (e) {
      // No selection persisted yet.
    }
  }

  FileView {
    id: openRouterModelsFile
    path: root.openRouterCacheDir + "/openrouter-models.json"
    watchChanges: true
    printErrors: false
    onLoaded: root.openRouterModels = root.parseOpenRouterModels(text())
  }

  FileView {
    id: openRouterSelectedFile
    path: root.openRouterStateDir + "/openrouter-selected-model.json"
    watchChanges: true
    atomicWrites: true
    printErrors: false
    onLoaded: root.parseOpenRouterSelection(text())
  }
  ...
  function launchAgent() {
    if (!root.bar) return
    if (root.isOpenRouter) root.bar.run("omarchy-agent-launch-openrouter")
    else root.bar.run("omarchy-agent --pick")
    root.close()
  }
    ...
    PanelKeyCatcher {
      ...
      Flickable {
        ...
        Column {
          ...
          PanelSeparator {
            visible: openRouterSection.visible
            foreground: root.foreground
          }
          ...
          Column {
            ...
            Text {
              width: parent.width
              text: root.openRouterSelectedModelId !== ""
                ? "Selected: " + root.openRouterSelectedModelName
            ...
            TextField {
              ...
              verticalPadding: Style.spacing.controlPaddingY
              onTextChanged: root.openRouterModelFilter = text
            }

            Text {
              visible: root.openRouterModels.length === 0
              width: parent.width
              text: "No cached model list yet. It refreshes automatically."
              color: root.dim
              font.family: root.fontFamily
              font.pixelSize: Style.font.caption
              wrapMode: Text.WordWrap
            ...
            Column {
              ...
              Repeater {
                model: root.openRouterFilteredModels
            ...
            Text {
              visible: root.openRouterSelectedModelId !== ""
              width: parent.width
              topPadding: Style.space(2)
              text: "Right-click the bar icon to launch this model."
              color: root.dim
              font.family: root.fontFamily
              font.pixelSize: Style.font.caption
              wrapMode: Text.WordWrap
  ...
  // A pickable model row for the OpenRouter launcher: same table-row look as
  // ModelRow, but clickable and highlighted by selection instead of a share bar.
  component OpenRouterModelRow: Item {
    id: orModelRow
    ...
    implicitHeight: orModelName.implicitHeight + Style.spacing.lg

    Rectangle {
      anchors.fill: parent
      radius: Style.cornerRadius
      color: orModelRow.selected ? root.alpha(root.foreground, 0.14)
        : (orHover.containsMouse ? root.alpha(root.foreground, 0.07) : "transparent")
    }

    Text {
      id: orModelName
      text: orModelRow.modelName
      color: root.foreground
      font.family: root.fontFamily
      font.pixelSize: Style.font.bodySmall
      elide: Text.ElideRight
